...
class Mantis:
    ...
    def update_motors(self,
                      motor_values: list,
                      sensor_data: list,
                      final_processing: str = "mod",
                      usage: str = "acceleration"):
        """
        Update motor positions based on the provided values
        Depending on the parameter, it can use the values as acceleration or target positions.
        At the same time, you can determine the final processing of the values.
        """
        if usage == "position":
            self.set_all_motors(motor_values, final_processing)
        elif usage == "sine":
            self.apply_sine(motor_values, final_processing)
        else:
            raise ValueError(f"Unknown usage type: {usage}. Use 'position' or 'sine'.")
    
    def set_all_motors(self,
                       positions: list,
                       final_processing: str):
        """Set the target position for all motors."""
        if final_processing:
            positions = self.process_positions(positions, final_processing)
    ...
    def process_positions(self,
                          positions: list,
                          option: str) -> list:
        """Process motor positions to ensure they are within limits."""
        if option == "mod":
            return [position % 6.28 for position in positions]
        elif option == "clip":
            import numpy as np
            return np.clip(positions, 0, 6.28).tolist()

    def apply_sine(self,
                   motor_values: list,
                   final_processing_function: str):
        ...
        for _ in range(GAIT_STEPS):
            ...
            self.move_motors_to_position(new_positions, final_processing_function, slices=SLICES)
    ...
    def move_motors_to_position(self,
        ...
        for slice_idx in range(1, slices + 1):
            ...
            self.set_all_motors(new_positions, final_processing_function)
            # Update last_positions for the next cycle.
            self.last_positions = new_positions[:]

            # In each time step, check if the robot is stable or not 
